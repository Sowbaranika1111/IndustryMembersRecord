const {PLMDevelopmentExpertise } = require("../../models/dropdownValuesModel.js");

// GET all expertise
const getAllExpertise = async (req, res) => {
  try {
    const expertises = await PLMDevelopmentExpertise.find().sort({ value: 1 });
...
const addSingleExpertise = async (req, res) => {
  try {
    ...
    const exists = await PLMDevelopmentExpertise.findOne({ value: { $regex: new RegExp(`^${trimmed}$`, "i") } });
    if (exists) {
      return res.status(400).json({ success: false, message: `Expertise "${exists.value}" already exists (case-insensitive match).` });
    }
    const newItem = new PLMDevelopmentExpertise({ value: trimmed });
...
const updateExpertiseById = async (req, res) => {
  try {
    ...
    const trimmed = value.trim();
    const existing = await PLMDevelopmentExpertise.findOne({ _id: { $ne: id }, value: { $regex: new RegExp(`^${trimmed}$`, "i") } });
    if (existing) {
      return res.status(400).json({ success: false, message: `Expertise "${existing.value}" already exists (case-insensitive match).` });
    }
    const updated = await PLMDevelopmentExpertise.findByIdAndUpdate(id, { value: trimmed }, { new: true, runValidators: true });
    if (!updated) {
...
const deleteExpertiseById = async (req, res) => {
  try {
    ...
    const deleted = await PLMDevelopmentExpertise.findByIdAndDelete(id);
    if (!deleted) {
      return res.status(404).json({ success: false, message: "Expertise not found." });
    }
...
const deleteAllExpertise = async (req, res) => {
  try {
    const deletionResult = await PLMDevelopmentExpertise.deleteMany({});
...
const insertAllExpertise = async (req, res) => {
  try {
    ...
    const existingDocs = await PLMDevelopmentExpertise.find({ value: { $in: uniqueInputValues } });
    ...
    const inserted = await PLMDevelopmentExpertise.insertMany(documentsToInsert);
